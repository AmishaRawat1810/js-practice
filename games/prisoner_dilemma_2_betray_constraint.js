...
function countChoice(playerHistory, move) {
  let count = 0;
  for (let index = 0; index < playerHistory.length; index++) {
    if (playerHistory[index] === move) {
      count++;
    }
  }
  return count;
...
function caughtBetrayer(playerMove, playerNum, playerHistory, range) {
  const count = countChoice(playerHistory, "b");
  let move = playerMove;

  if (move === 'b' && count >= range) {
    display("You can't betray this time...😈");
    move = promptMove(playerNum);
    if (move === 'b') {
      return caughtBetrayer(move, playerNum, playerHistory, range);
    }
  }
  display("I'll catch you betraying next time... 🪤")
  return move;
}

function caughtCooperater(playerMove, playerNum, playerHistory, range) {
  const count = countChoice(playerHistory, "c");
  let move = playerMove;

  if (move === 'c' && count >= range) {
    display("You can't coorporate this time...😈");
    move = promptMove(playerNum);
    if (move === 'c') {
      return caughtBetrayer(move, playerNum, playerHistory, range);
    }
  }
  display("I'll catch you cooperating next time... 🪤")
  return move;
}
...
function playGame() {
  ...
  let rounds = roundsToPlay(5)
  let minBetray = roundsToPlay(2);
  const player1History = []
  const player2History = []

  for (let round = 1; round <= rounds; round++) {
    display(`🔔 Round ${round} :`)
    let player1Move = promptMove(1);
    player1Move = caughtBetrayer(player1Move, 1, player1History, minBetray);
    delayTheText(10);

    let player2Move = promptMove(2);
    player2Move = caughtBetrayer(player2Move, 2, player2History, minBetray);
    delayTheText(10);
